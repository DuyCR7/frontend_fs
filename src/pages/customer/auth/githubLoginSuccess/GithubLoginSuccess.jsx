...
import {signInGithubSuccess} from "../../../../services/customer/authService";
...
const GithubLoginSuccess = (props) => {

    const navigate = useNavigate();
    const customer = useSelector((state) => state.customer);
    const { userId, tokenLoginGithub } = useParams();
    const dispatch = useDispatch();
    const [isLoading, setIsLoading] = useState(true);

    useEffect(() => {
        fetchSignInGithubSuccess();
    }, []);

    const fetchSignInGithubSuccess = async () => {
        try {
            let res = await signInGithubSuccess(userId, tokenLoginGithub);
            if (res && res.EC === 0) {
                let id = res.DT.id;
